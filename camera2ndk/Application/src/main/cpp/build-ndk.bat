...
rem Outputs are copied to camera-engine\src\main\jniLibs\<abi> for prebuilt /
rem split-repo consumption (Gradle skips CMake when cpp sources are absent).
rem
rem Usage:
...
set "ENGINE_DIR=%PROJECT_ROOT%camera-engine"
set "LOCAL_PROPERTIES=%PROJECT_ROOT%local.properties"
set "CMAKE_LISTS=%APP_DIR%\CMakeLists.txt"
set "JNI_LIBS_DIR=%ENGINE_DIR%\src\main\jniLibs"
set "APP_JNI_LIBS_DIR=%APP_DIR%\src\main\jniLibs"
...
echo [INFO] Engine jniLibs: %JNI_LIBS_DIR%
...
echo [INFO] Tip: when Application/src/main/cpp is removed (split repo),
echo [INFO]       Gradle will pick these prebuilt libs automatically.
...
rem Keep OpenCV runtime next to the engine for split-repo / prebuilt mode.
...
rem Also refresh App jniLibs OpenCV copy used by the current monorepo layout.
if not exist "%APP_JNI_LIBS_DIR%\%ABI%" mkdir "%APP_JNI_LIBS_DIR%\%ABI%"
copy /Y "%OPENCV_SO%" "%APP_JNI_LIBS_DIR%\%ABI%\libopencv_java4.so" >nul

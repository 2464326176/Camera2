...
class SoftwarePipeline(width: Int, height: Int, fps: Int, filterOn: Boolean,
    ...
    /** Builds a preview capture request targeting the preview SurfaceView. */
    ...
    /** Builds a record capture request targeting both the preview and encoder surfaces. */
    ...
    /** Returns the preview Surface as the only preview target. */
    override fun getPreviewTargets(): List<Surface> {
        return listOf(viewFinder.holder.surface)
    }

    /** Returns the preview Surface and the encoder input surface as record targets. */

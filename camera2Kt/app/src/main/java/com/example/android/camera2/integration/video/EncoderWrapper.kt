...
package com.example.android.camera2.integration.video
...
import com.example.android.camera2.integration.video.fragments.VideoCodecFragment
...
class EncoderWrapper(width: Int,
    ...
    /** Target video encoding bit rate, in bits per second. */
    private val mBitRate = bitRate
    /** Target video frame rate, in frames per second. */
    private val mFrameRate = frameRate
    /** Dynamic range profile used for HDR encoding (e.g. HLG10, HDR10). */
    private val mDynamicRange = dynamicRange
    /** Clockwise rotation hint applied to the output video, in degrees. */
    private val mOrientationHint = orientationHint
    /** File the encoded video is written to. */
    private val mOutputFile = outputFile
    /** Whether to use MediaRecorder instead of the lower-level MediaCodec path. */
    private val mUseMediaRecorder = useMediaRecorder
    /** Selected video codec id (e.g. H264, HEVC, AV1). */
    private val mVideoCodec = videoCodec

    /** MIME type derived from the selected video codec. */
    ...
    /** MediaRecorder instance, only used when [mUseMediaRecorder] is true. */
    private var mMediaRecorder: MediaRecorder? = null

    /** Builds and configures a [MediaRecorder] writing to [mOutputFile] for the given surface. */
    ...
    /** Maps the configured [mDynamicRange] to a MediaFormat color transfer value. */
    ...
    /** Starts encoding: prepares/starts MediaRecorder or launches the [EncoderThread]. */
    public fun start() {
    ...
    /** Blocks until the first encoded frame has been produced (MediaCodec path only). */
    ...
    private class EncoderThread(mediaCodec: MediaCodec,
        ...
        /** The MediaCodec instance that produces encoded buffers. */
        val mEncoder = mediaCodec
        /** MediaFormat reported by the codec once encoding actually starts. */
        var mEncodedFormat: MediaFormat? = null
        /** Reusable buffer info structure describing each output buffer. */
        val mBufferInfo = MediaCodec.BufferInfo()
        /** MediaMuxer that writes the encoded stream into the output MP4 file. */
        val mMuxer = MediaMuxer(outputFile.getPath(), MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4)
        /** Clockwise rotation hint applied to the muxed video, in degrees. */
        val mOrientationHint = orientationHint
        /** Index of the video track added to the muxer (-1 until added). */

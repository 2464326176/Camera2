...
package com.example.android.camera2.integration.cameraxext.ui
...
import com.example.android.camera2.integration.cameraxext.adapter.CameraExtensionsSelectorAdapter
import com.example.android.camera2.integration.cameraxext.model.CameraUiAction
import com.example.android.camera2.integration.cameraxext.viewstate.CameraPreviewScreenViewState
...
class CameraExtensionsScreen(private val root: View) {
    ...
    /** Merges the camera-preview and post-capture parts of [state] and refreshes the screen. */
    ...
    /** Shows a long Toast containing the given capture [errorMessage]. */
    fun showCaptureError(errorMessage: String) {
        Toast.makeText(context, errorMessage, Toast.LENGTH_LONG).show()
    }

    /** Hides the permissions-rationale container. */
    fun hidePermissionsRequest() {
        permissionsRationaleContainer.isVisible = false
    }

    /** Shows the permissions-rationale container, choosing the right copy for the situation. */
    fun showPermissionsRequest(shouldShowRationale: Boolean) {
        ...
        if (shouldShowRationale) {
            permissionsRationale.text =
                context.getString(R.string.camera_permissions_request)
        } else {
    ...
    /** Smoothly scrolls so the tapped extension item is centered in the selector. */
    ...
    /** Handles the lens-switch button: plays a 180° flip animation then emits a switch event. */

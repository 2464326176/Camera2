...
package com.example.android.camera2.integration.extensions.fragments
...
import com.example.android.camera2.integration.extensions.ZoomUtil
import com.example.android.camera2.integration.databinding.ExtFragmentCameraBinding
...
class CameraFragment : Fragment(), TextureView.SurfaceTextureListener {
   ...
   */
  /** The currently selected camera extension mode id (or -1 when none). */
  private var currentExtension = -1
  /** Index of the selected extension within the extension list (or -1 when none). */
   ...
   */
  private var _binding: ExtFragmentCameraBinding? = null
  private val binding get() = _binding!!

  /** Current zoom ratio applied to the preview (1.0f means no zoom). */
  ...
  override fun onCreateView(
    ...
    _binding = ExtFragmentCameraBinding.inflate(inflater, container, false)
    return binding.root
  }
  ...
  /** Decodes an [Image] into a Bitmap (JPEG directly, YUV via [YuvToRgbConverter]). */
  ...
  /** Enables or disables the shutter, switch, and strength UI controls on the main thread. */
  ...
  /** Single-surface overload that forwards to [submitRequest] with a one-element target list. */
  ...
  /** Builds a CaptureRequest from [templateType] and [targets] and submits it (repeating or once). */
  private fun submitRequest(
    templateType: Int,
  ...
  /** Submits a one-shot preview request that triggers auto-focus at [meteringRectangle]. */
  ...
  /** Cancels any pending auto-focus reset and resumes continuous auto-focus. */
  ...
  /** Computes and applies a new zoom ratio from a pinch [scaleFactor], clamped to valid range. */

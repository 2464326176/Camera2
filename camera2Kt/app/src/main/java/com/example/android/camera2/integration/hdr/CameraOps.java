...
public class CameraOps {
    ...
    /** System camera manager used to open cameras. */
    private final CameraManager mCameraManager;
    /** Currently open camera device, or null when closed. */
    private CameraDevice mCameraDevice;
    /** Active capture session, or null before configuration completes. */
    private CameraCaptureSession mCameraSession;
    /** Output surfaces configured for the current capture session. */
    private List<Surface> mSurfaces;

    /** Condition variable used to block until the camera is fully closed. */
    private final ConditionVariable mCloseWaiter = new ConditionVariable();

    /** Background thread that hosts all camera operations. */
    private HandlerThread mCameraThread;
    /** Handler bound to [mCameraThread] for posting camera operations. */
    private Handler mCameraHandler;

    /** Callback used to surface error messages to the UI. */
    private final ErrorDisplayer mErrorDisplayer;

    /** Listener notified when the camera is ready for capture requests. */
    private final CameraReadyListener mReadyListener;
    /** Handler used to deliver [mReadyListener] callbacks on the main thread. */
